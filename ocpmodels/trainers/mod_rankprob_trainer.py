...
@registry.register_trainer("rankprob")
class EnergyTrainer(BaseTrainer):
    ...
    def _compute_loss(self, out, batch_list):
        ...
        #import pdb; pdb.set_trace()
        #batch_list <- list type
        #for batch in batch_list --> batch: torch_gemetric class
        #collect y_relaxed from batch list, size of energy_target = size of batch
        import itertools
        batch_size = energy_target.size(dim=0)
        ensemble = torch.cat(
            [batch.y_ens.to(self.device) for batch in batch_list], dim=0
        )
        ens = ensemble.reshape(batch_size,-1) # (batch size, ensemble size)
        ens.requires_grad = True
        ens = ens.to(torch.float32)
        ens_size = ens.shape[1]
        idx_combo = list(itertools.combinations(np.arange(len(ens)), r=2))
        # d_ens = []
        # for combo in idx_combo:
        #     d_ens.append(ens[combo[0]]-ens[combo[1]])
        # d_ens = torch.stack(d_ens)
        stds = []
        for combo in idx_combo:
            s = (ens[combo[0]]-ens[combo[1]]).std()
            stds.append(s)
        stds = torch.stack(stds)
        
        # energy_combo = torch.combinations(energy_target, r=2)
        # energy_delta = energy_combo[:,0] - energy_combo[:,1]
        # energy_delta = torch.reshape(energy_delta, (len(energy_delta,-1))
        # y_ens = energy_delta + d_ens
        ...
        Py = torch.exp(target_delta/stds)/torch.exp(target_delta/stds).sum()
        Pz = torch.exp(out_delta)/torch.exp(out_delta).sum()
                                     
        # Py = torch.exp(energy_delta)/torch.exp(energy_delta).sum()
        # Pz = torch.exp(out['energy'])/torch.exp(out['energy']).sum()
        loss = (-Py*torch.log(Pz)).sum()
