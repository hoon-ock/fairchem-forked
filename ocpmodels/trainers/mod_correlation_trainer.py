...
@registry.register_trainer("correlation")
...
#         return loss
    ...
    def _compute_loss(self, out, batch_list):
        ...
        # ens_norm = ens # not normalized by std
        ens_norm = (ens - ens.mean(1, True))/ens.std(1, True).reshape(-1,1)
        corr_matrix = ens_norm @ ens_norm.t()/(ens_size-1)
        inv_corr_matrix = torch.inverse(corr_matrix)
        ##################################################################
        if self.normalizer.get("normalize_labels", False):
            target_normed = self.normalizers["target"].norm(energy_target)
        else:
            target_normed = energy_target
        #################################################################
        dE = target_normed - out['energy']
        dE = dE.reshape(-1,1)      
        loss = (dE.t() @ inv_corr_matrix @ dE)/batch_size
...
#     import yaml
#     # with open("/home/jovyan/ocp/configs/s2ef/all/gemnet/gemnet-dT.yml", 'r') as f:
#     #     config = yaml.safe_load(f)

#     # with open("/home/jovyan/ocp/configs/is2re/all/base.yml", 'r') as f:
#     #     base = yaml.safe_load(f)
#     with open("/home/jovyan/ocp/configs/is2re/example.yml", 'r') as f:
#         config = yaml.safe_load(f)
#     print(config.keys())
#     trainer = EnergyTrainer(task=config['task'], 
#                             model=config['model'], 
