...
@registry.register_trainer("sigma")
class EnergyTrainer(BaseTrainer):
    ...
    def _compute_loss(self, out, batch_list):
        ...
        idx_combo = list(itertools.combinations(np.arange(batch_size), r=2)) #np.arange(len(ens))
        #ens_diff = []
        stds = []
        if len(idx_combo)==0:
            s=ens.std()
            ...
            #ens_diff.append(ens[combo[0]]-ens[combo[1]])
        
        try:
            sigma = torch.stack(stds)
        except:
            print(batch_size)
            print(ens)
            print(idx_combo) #idx_combo --> empty <<-- should resolve this issue
            print(stds)
            print(ens)
            
        del stds
        #ens_diff = torch.stack(ens_diff)
        #stds_ = torch.std(ens_diff,1)
        ##################################################################
        if self.normalizer.get("normalize_labels", False):
            target_normed = self.normalizers["target"].norm(energy_target)
            #target_stds = self.normalizers["target"].norm(stds)**15*10
        else:
            target_normed = energy_target
        ##################################################################
        target_combo = torch.combinations(target_normed, r=2)
        target_delta = target_combo[:,0] - target_combo[:,1]
        #target_delta = target_delta[stds<0.25]
        
        out_combo = torch.combinations(out["energy"], r=2)
        out_delta = out_combo[:,0] - out_combo[:,1]
        #out_delta = out_delta[sigma<0.25]
        '''rectified mode'''
        sigma = (sigma)**5
        ''''''''''''''''''''
        #loss = abs((target_delta - out_delta)/sigma).mean() #l1
        #loss = (((target_delta - out_delta)/sigma)**2).mean() #l2
        loss = (((target_delta - out_delta)/sigma)**2).mean() #l2
        #loss = sum((((target_delta - out_delta))**2))/sum(sigma<1) #l2
        # loss = self.loss_fn["energy"](out_delta/sigma, target_delta/sigma) #L1
