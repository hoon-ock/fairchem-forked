...
#     std_y  = np.std ( y )
#     n      = len    ( x.T ) 
#     return np.correlate( x - mean_x, y-mean_y, mode = 'valid' )[0] / (n) 

def emb_corrcoef(emb_i, emb_j):
    emb_ij = torch.cat((emb_i, emb_j),0)
    # cor_matrix = torch.cov(emb_ij) 
    #import pdb; pdb.set_trace()
    cor_matrix=(emb_ij-torch.mean(emb_ij,1,True))@(emb_ij-torch.mean(emb_ij,1,True)).t()
    cor_ii = cor_matrix[:len(emb_i), :len(emb_i)].sum() 
    cor_jj = cor_matrix[len(emb_i):,len(emb_i):].sum() 
    cor_ij = cor_matrix[:len(emb_i), len(emb_i):].sum() 
    COR = cor_ij / (torch.sqrt(cor_ii)*torch.sqrt(cor_jj))
    return COR

@registry.register_trainer("emb")
class EnergyTrainer(BaseTrainer):
    ...
    def _compute_loss(self, out, batch_list):
        ...
        emb = out["embedding"]
        idx_combo = list(itertools.combinations(np.arange(batch_size), r=2)) 
        #import pdb; pdb.set_trace()
        sims = []
        for combo in idx_combo:
            s = emb_corrcoef(emb[combo[0]], emb[combo[1]])
            s = (1+s)
            #s = torch.exp(s)
            sims.append(1/s)

        #import pdb; pdb.set_trace()
        sigma = torch.stack(sims)
        del sims
        del emb
        ##################################################################
