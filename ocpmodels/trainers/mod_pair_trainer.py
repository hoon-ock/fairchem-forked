...
@registry.register_trainer("pair")
...
#         key = [ads_combo[:,0] == ads_combo[:,1]] or [bulk_combo[:,0] == bulk_combo[:,1]]
        ...
        y_ = torch.sigmoid(out_delta*10) #std**3
        
        loss = -(y*torch.log(y_) + (1-y)*torch.log(1-y_)).sum() #Binary Cross Entropy
        #loss = torch.maximum(1.0-(out_delta / target_delta) , target_delta * 0.).sum()
        #loss = -2*torch.sigmoid(2* out_delta / target_delta).sum()
        #loss = 10000*torch.maximum(-target_delta * out_delta, target_delta * 0.).sum() # Hinge loss
