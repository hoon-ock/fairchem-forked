...
@registry.register_model("equiformer_v2_embedding_head")
class EquiformerV2EnergyHead(nn.Module, HeadInterface):
    def __init__(self, backbone, reduce: str="max"):
        super().__init__()
        self.reduce = reduce

    def forward(self, data: Batch, emb: dict[str, torch.Tensor | GraphData]):
        x = emb["node_embedding"]
        h = x.embedding.view(len(data.batch), -1) # [num_atoms, (maximum degree of the spherical harmonics+1)**2*num_sphere_channels]
        h = scatter_det(
                h, data.batch, dim=0, dim_size=torch.max(data.batch)+1, reduce=self.reduce
            )
        return {"emb": h}
